const DEFAULT_AI_BASE = "https://space.ai-builders.com/backend";
const DEFAULT_AI_MODEL = "grok-4-fast";

export function createQuickPick(env = {}) {
  const aiBase = env.AI_BUILDER_API_BASE || DEFAULT_AI_BASE;
  const aiToken = env.AI_BUILDER_TOKEN || "";
  const aiModel = env.AI_BUILDER_MODEL || DEFAULT_AI_MODEL;
  const fallbackModel = env.AI_BUILDER_FALLBACK_MODEL || DEFAULT_AI_MODEL;
  const debug = env.DEBUG_AI === "1";
  ...
  async function handleSession(payload = {}) {
    ...
    if (candidates.length < 3 || candidates.length > 6) {
      return {
        status: 400,
        body: { error: "Please provide between 3 and 6 candidates." },
      };
    }
    ...
    if (!aiToken) {
      return {
        status: 200,
        body: buildFallbackPlan(context, {
    ...
    try {
      ...
      return { status: 200, body: { status: "ready", plan: normalized } };
    } catch (error) {
  ...
  async function handleNext(payload = {}) {
    ...
    if (!aiToken) {
      return {
        status: 200,
        body: buildFallbackResponse(context, {
  ...
  async function handleResult(payload = {}) {
    ...
    if (!aiToken) {
      return {
        status: 200,
        body: buildResultFallback(context, ranking, {
  ...
  async function callAiJson({ systemPrompt, userPrompt, maxTokens }) {
    const modelsToTry = aiModel && aiModel !== fallbackModel
    ...
    for (const model of modelsToTry) {
      try {
        const response = await fetch(`${aiBase}/v1/chat/completions`, {
          method: "POST",
          headers: {
